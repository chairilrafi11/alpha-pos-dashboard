import { InvoiceProductDetail } from "@/types/invoice/invoice";
import { formatIDR } from "@/utils/currencyFormatter";

export default function InvoiceDetailTable({ details }: { details: InvoiceProductDetail[] }) {
  return (
        ...
        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
          {
            details?.map((detail, index) => (
              <tr key={index}>
                ...
                <td className="px-5 py-3 text-center text-sm text-gray-500 dark:text-gray-400">
                  {formatIDR(detail.discount_price)}
                </td>
