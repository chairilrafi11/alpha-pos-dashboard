// src/components/GlobalModalRenderer.tsx
'use client';

import React, { useEffect } from 'react';
import { useGlobalModal } from '@/context/ModalContext';
import { Modal } from '../ui/modal';
// Asumsikan komponen Modal Anda di-import di sini
// import Modal from '@/components/common/Modal';

export default function GlobalModalRenderer() {
    const { isOpen, closeModal, content, title, onConfirm, autoCloseDelay } = useGlobalModal();

    // --- Logic Auto-Close (useEffect) ---
    useEffect(() => {
        let timer: NodeJS.Timeout | null = null;

        // Jika modal terbuka DAN autoCloseDelay > 0
        if (isOpen && autoCloseDelay && autoCloseDelay > 0) {
            // Set timer untuk menutup modal
            timer = setTimeout(() => {
                closeModal();
                if (onConfirm != null) {
                    onConfirm();
        ...
        // Cleanup function: Hapus timer jika modal ditutup secara manual 
        // atau jika komponen dilepas/state berubah
        return () => {
            if (timer) {
                clearTimeout(timer);
            }
    ...
    }, [isOpen, autoCloseDelay, closeModal]); // Dependencies
