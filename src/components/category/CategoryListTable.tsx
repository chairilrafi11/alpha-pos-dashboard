...
export default function CategoryListTable() {
  ...
  const handleDelete = (selected: Category) => {
    openModal({
  ...
  const sortedCategories = () => {
    return [...categories.data].sort((a, b) => {
      let valA = a[sort.key];
      let valB = b[sort.key];
      if (sort.key === "threshold_harga_percentage") {
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-gray-200 dark:divide-gray-800 dark:border-gray-800">
              ...
              <th
                onClick={() => sortBy("threshold_harga_percentage")}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "threshold_harga_percentage" && sort.asc
                          ? "text-gray-500 dark:text-gray-400"
                          : "text-gray-300 dark:text-gray-400/50"
                      }
                      width="8"
          ...
          <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
            {paginatedCategories().map((category, index) => (
              ...
              >
                ...
                <td className="px-5 py-4 whitespace-nowrap">
                  <span
                    className={`text-xs rounded-full px-2 py-0.5 font-medium ${category.status === "active"
                  ...
                  <div className="relative flex justify-center dropdown">
                    <TableDropdown
                      ...
                      dropdownContent={
                        <>
                          <button
                            onClick={() => router.push(`/category/${encodeId(category.id)}`)}
                          ...
                          <button
                            onClick={() => router.push(`/category/${encodeId(category.id)}/edit`)}
                            className="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
                          >
                            Edit
                          </button>
