...
import { getShiftingHistories } from "@/services/shiftingHistoryService";
import Badge from "../ui/badge/Badge";
import { getRoleBadgeColor, getShiftingStatusBadgeColor } from "@/utils/colors";
import { getShiftingStatusName } from "@/utils/textFormatter";

interface Sort {
  key: keyof ShiftingHistory;
  asc: boolean;
}
        ...
        <div className="absolute right-0 z-10 mt-2 w-56 rounded-lg border border-gray-200 bg-white p-4 shadow-lg dark:border-gray-700 dark:bg-gray-800">
          <div className="mb-5">
            ...
            <input
              ...
              placeholder="Search shiftingHistory..."
            />
          </div>
          <div className="mb-5">
            <label className="mb-2 block text-xs font-medium text-gray-700 dark:text-gray-300">
...
export default function ShiftingHistoryListTable() {
  const router = useRouter();
  const { openModal } = useGlobalModal();
  const [shiftingHistories, setShiftingHistories] = useState<PaginatedResponse<ShiftingHistory>>({
    data: [],
  ...
  const fetchData = useCallback(
    async () => {
      setIsLoading(true);
      try {
        const data = await getShiftingHistories(params);
        setShiftingHistories(data);
      } catch (error) {
      } finally {
        setIsLoading(false);
      }
  ...
  const handleDelete = (selected: ShiftingHistory) => {
    openModal({
      title: "Delete Shifting History",
      content: `Are you sure you want to delete ${selected.name} - Branch ${selected.branch_name} ?`,
      type: "confirm",
      onConfirm: () => {
  ...
  const handleDeleteInvoice = async (id: number) => {
    try {
      // TODO : delete shiftingHistory
      fetchData();

    } catch (error) {
      console.error("Gagal menghapus shiftingHistory:", error);
    }
  };

  const handlePageChange = (page: number) => {
    if (page >= 1 && page <= shiftingHistories.page_info.total_page) {
      setParams(prev => ({
        ...prev,
        page: page
      }));
  ...
  const sortedCategories = () => {
    return [...shiftingHistories.data].sort((a, b) => {
      let valA = a[sort.key];
      let valB = b[sort.key];
      if (sort.key === "opening_amount" || sort.key === "closing_amount" || sort.key === "total_invoice") {
  ...
  const sortBy = (key: keyof ShiftingHistory) => {
    setSort((prev) => ({
      key,
      ...
      <div className="flex flex-col justify-between gap-5 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center dark:border-gray-800">
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            variant="primary"
            endIcon={<PlusIcon />}
            onClick={() => router.push('/shifting-history/create')}
          >
            Add
          </Button>
        </div>
      ...
      <div className="overflow-x-auto custom-scrollbar">
        <table className="w-full">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "opening_amount" && sort.asc
                          ? "text-gray-500 dark:text-gray-400"
                          : "text-gray-300 dark:text-gray-400/50"
                      }
                      width="8"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "closing_amount" && sort.asc
                          ? "text-gray-500 dark:text-gray-400"
                          : "text-gray-300"
                      }
                      width="8"
          ...
          <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
            {paginatedCategories().map((shiftingHistory, index) => (
              <tr
                key={shiftingHistory.id}
                className="transition hover:bg-gray-50 dark:hover:bg-gray-900"
              >
                <td className="w-14 px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {getNoRow(index, shiftingHistories.page_info.page_number, shiftingHistories.page_info.page_size)}
                  </p>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <span className="text-theme-sm font-medium text-gray-700 group-hover:underline dark:text-gray-400">
                    {shiftingHistory.name}
                  </span>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    {formatIDR(shiftingHistory.opening_amount)}
                  </p>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    {formatIDR(shiftingHistory.closing_amount)}
                  </p>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    {formatIDR(shiftingHistory.revenue)}
                  </p>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    {formatIDR(shiftingHistory.selisih)}
                  </p>
                </td>
                <td className="px-5 py-4">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    <Badge
                      size="sm"
                      color={getShiftingStatusBadgeColor(shiftingHistory.status)}
                    >
                      {getShiftingStatusName(shiftingHistory.status)}
                    </Badge>
                  </p>
                </td>
                <td className="px-5 py-4">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    {shiftingHistory.branch_name}
                  </p>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-700 dark:text-gray-400">
                    {dateFormatReadable(shiftingHistory.created_at)}
                  </p>
                </td>
                  ...
                  <div className="relative flex justify-center dropdown">
                    <TableDropdown
                      ...
                      dropdownContent={
                        <>
                          <button
                            onClick={() => router.push(`/shifting-histories/${encodeId(shiftingHistory.id)}`)}
                          ...
                          <button
                            onClick={() => router.push(`/shifting-histories/${encodeId(shiftingHistory.id)}/edit`)}
                            className="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
                          >
                            Edit
                          </button>
                          <button
                            onClick={() => handleDelete(shiftingHistory)}
                            className="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300">
                            Delete
                          </button>
                        </>
        ...
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="pb-3 xl:pb-0">
            <p className="pb-3 text-sm font-medium text-center text-gray-500 border-b border-gray-100 dark:border-gray-800 dark:text-gray-400 xl:border-b-0 xl:pb-0 xl:text-left">
              Showing {shiftingHistories.page_info.page_number} to {shiftingHistories.page_info.total_page} of {shiftingHistories.page_info.total_data} entries
            </p>
          </div>
          <Pagination
            currentPage={shiftingHistories.page_info.page_number}
            totalPages={shiftingHistories.page_info.total_page}
