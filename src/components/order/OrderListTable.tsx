...
import { getOrderStatusBadgeColor, getOrderWorkflowBadgeColor } from "@/utils/colors";
...
export default function OrderListTable() {
  ...
  const [sort, setSort] = useState<Sort>({ key: "id", asc: true });
  ...
  const handleDelete = (selected: Order) => {
    openModal({
      title: "Delete Order",
      content: `Are you sure you want to delete Order Customer ${selected.customer_name} ?`,
      type: "confirm",
      onConfirm: () => {
        handleDeleteOrder(selected.id);
      },
    })
  }
  ...
  const handleDeleteOrder = async (id: number) => {
    try {
      ...
      console.error("Gagal menghapus orders:", error);
    }
  };

  const handlePageChange = (page: number) => {
    if (page >= 1 && page <= orders.page_info.total_page) {
      setParams(prev => ({
        ...prev,
        page: page
      }));
  ...
  const sortedCategories = () => {
    return [...orders.data].sort((a, b) => {
      let valA = a[sort.key];
      let valB = b[sort.key];
      if (sort.key === "total_amount" || sort.key === "total_item") {
  ...
  const handleRowClick = (orderId: number) => {
    const hashedId = encodeId(orderId);
    router.push(`/orders/${hashedId}`);
  };
      ...
      <div className="overflow-x-auto custom-scrollbar">
        <table className="w-full">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "customer_name" && sort.asc
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "total_amount" && sort.asc
                          ? "text-gray-500 dark:text-gray-400"
                          : "text-gray-300 dark:text-gray-400/50"
                      }
                      width="8"
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="flex flex-col gap-0.5">
                    <svg
                      className={
                        sort.key === "workflow_name" && sort.asc
                          ? "text-gray-500 dark:text-gray-400"
                          : "text-gray-300"
                      }
                      width="8"
          ...
          <tbody className="divide-x divide-y divide-gray-200 dark:divide-gray-800">
            {paginatedCategories().map((order, index) => (
              <tr
                key={order.id}
              ...
              >
                <td className="w-14 px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {getNoRow(index, orders.page_info.page_number, orders.page_info.page_size)}
                  </p>
                </td>
                ...
                <td className="px-5 py-4 whitespace-nowrap">
                  <Badge color={getOrderWorkflowBadgeColor(order.workflow_name)}>
                    {order.workflow_name}
                  </Badge>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                  <Badge color={getOrderStatusBadgeColor(order.status)}>
                    {order.status}
                  </Badge>
                </td>
                <td className="px-5 py-4 whitespace-nowrap">
                ...
                <td className="px-5 py-4 whitespace-nowrap">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {dateFormatReadable(order.created_at)}
                  </p>
                </td>
                  ...
                  <div className="relative flex justify-center dropdown">
                    <TableDropdown
                      ...
                      dropdownContent={
                        <>
                          <button
                            onClick={() => router.push(`/orders/${encodeId(order.id)}`)}
                          ...
                          <button
                            onClick={() => router.push(`/orders/${encodeId(order.id)}/edit`)}
                            className="text-xs flex w-full rounded-lg px-3 py-2 text-left font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700 dark:text-gray-400 dark:hover:bg-white/5 dark:hover:text-gray-300"
                          >
                            Edit
                          </button>
        ...
        <div className="flex flex-col xl:flex-row xl:items-center xl:justify-between">
          ...
          <div className="pb-3 xl:pb-0">
            <p className="pb-3 text-sm font-medium text-center text-gray-500 border-b border-gray-100 dark:border-gray-800 dark:text-gray-400 xl:border-b-0 xl:pb-0 xl:text-left">
              Showing {orders.page_info.page_number} to {orders.page_info.total_page} of {orders.page_info.total_data} entries
            </p>
          </div>
          <Pagination
            currentPage={orders.page_info.page_number}
            totalPages={orders.page_info.total_page}
